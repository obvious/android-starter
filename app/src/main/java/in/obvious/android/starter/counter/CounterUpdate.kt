...
class CounterUpdate(
  ...
  override fun update(model: CounterModel, event: CounterEvent): Next<CounterModel, CounterEffect> {
    return when (event) {
      ...
      StartCountdownClicked ->
        next(model.countdownStarted(), setOf(StartCountdown(model.counterValue, SECONDS)))
  ...
  private fun incrementCounter(model: CounterModel): Next<CounterModel, CounterEffect> {
    return if (model.counterValue < maxAllowedCounterValue) next(model.increment()) else noChange()
  }

  private fun decrementCounter(model: CounterModel): Next<CounterModel, CounterEffect> {
    return if (model.counterValue > minAllowedCounterValue) next(model.decrement()) else noChange()
  }
}
